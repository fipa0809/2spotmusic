...
// get top tracks
			...
			var getData = $.getJSON(spotifyArtists + artistId + spotifyTopTracks);
//append tracks
	...
	getData.then(function(data) {
		...
		for (var i = 0; i < 5; i++) {
			...
			$('ul').append('<li style="list-style:none" class="trackItem"'+ '>' + data.tracks[i].name + ' <button type="text"' +
				' class="play-button" id="' + data.tracks[i].id + '">Play</button></li>');
		}
	...
	// checks if anything in array, if there is, pause and clear it to play next song
		...
		$('ul').on('click', '.play-button', function() {
